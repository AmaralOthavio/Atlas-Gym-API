...
def informar_verificacao(tipo=0, trazer_pl=False):
    ...
    if verificacao == 1:
        return jsonify({'message': 'Token de autenticação necessário.', "verificacao": verificacao}), 401
    elif verificacao == 2:
        return jsonify({'message': 'Token expirado.', "verificacao": verificacao}), 401
    elif verificacao == 3:
        return jsonify({'message': 'Token inválido.', "verificacao": verificacao}), 401
    elif verificacao == 4:
        return jsonify({'message': 'Nível Personal Trainer requerido.', "verificacao": verificacao}), 401
    elif verificacao == 5:
        return jsonify({'message': 'Nível Administrador requerido.', "verificacao": verificacao}), 401
    else:
        if trazer_pl:
            return verificacao
        return None
...
def cadastrar_cliente():
    ...
    if ano_nasc > hoje_ano or hoje_ano - ano_nasc < 17:
        return jsonify({"message": "Data de nasicmento inválida"}), 401
    if len(nome) > 895:
...
def cadastrar_personal_trainer():
    ...
    if '@' not in email:
        return jsonify({"message": "E-mail inválido"}), 401
    if len(form) > 1000:
        return jsonify({"message": "Limite de caracteres de formação excedido (1000)"}), 401
    ...
    try:
        ...
        if resposta:
            if resposta[0] == cref:
                return jsonify({"message": "Registro de CREF já cadastrado"}), 401
        ...
        con.commit()
        return jsonify({"message": "Personal Trainer cadastrado com sucesso!"}), 200
...
def cadastrar_administrador():
    ...
    try:
        ...
        con.commit()
        return jsonify({"message": "Administrador cadastrado com sucesso!"}), 200
